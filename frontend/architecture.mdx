---
title: "Architecture"
description: "The four-entity trust model and how the frontend fits into the full voting system."
---

The frontend is a Next.js 15 application deployed on Vercel. It is the only surface voters and admins interact with directly. The cryptographic voting protocol is handled server-side by the backend — the browser's role is to collect voter input, communicate with the API, and display results.

## Stack

| Layer | Technology |
|---|---|
| Framework | Next.js 15, App Router |
| Language | TypeScript |
| Styling | Tailwind CSS v4 |
| Components | shadcn/ui + Radix UI |
| Server state | TanStack Query v5 |
| Client state | Zustand |
| Forms | React Hook Form + Zod |
| HTTP | Axios |
| Charts | Recharts |
| Toasts | Sonner |

---

## Four-entity trust model

```mermaid
graph TD
    Voter["Voter (Browser)"]
    Admin["Administrator"]
    Commissioner["Commissioner"]
    Counter["Counter"]

    Voter -->|"N1 eligibility check"| Commissioner
    Voter -->|"blinded ballot"| Admin
    Admin -->|"blind signature s'"| Voter
    Voter -->|"encrypted ciphertext C"| Counter
    Counter -->|"public key"| Voter

    note1["Voter unblinds s' locally\nAdministrator never sees ballot content"]
```

| Entity | Knows | Does not know |
|---|---|---|
| Voter | Their own N1, N2, vote choice, blinding factor k | Other voters' choices |
| Administrator | Voter identity via N1/commissioner check | Vote content (blind signatures) |
| Commissioner | Valid N1 list and N2 hashes | Vote choices or ballot content |
| Counter | Decrypted vote content | Which voter cast which ballot |

---

## Folder structure

```
src/
├── app/
│   ├── page.tsx                   # landing page
│   ├── layout.tsx
│   ├── globals.css
│   ├── admin/                     # /admin
│   ├── register/                  # /register
│   ├── vote/                      # /vote
│   ├── results/                   # /results
│   │   └── verify-vote/           # /results/verify-vote
│   ├── theme/                     # /theme
│   └── [slug]/                    # static pages (about, policies)
├── components/
│   ├── Admin/                     # AdminDashboard, AdminResults, ConfigEditor, VoteControls
│   ├── Vote/                      # AuthStep, VoteStep, VoteStarted, VoteNotStarted, VoteEnded, VoteSubmitted, VoteTheme, Stepper
│   ├── Landing/                   # Hero, Features, Faqs, Team, TechStack, VotingTimeline
│   ├── Results/                   # VotingResults, VerifyMyVote
│   └── ui/                        # shared shadcn/ui primitives
├── hooks/                         # useVoteStatus, useSubmitVote, useVerifyVote, useStartVote, useEndVote, ...
├── lib/
│   ├── api/                       # one file per endpoint: auth, submitVote, votingStatus, votingTheme, verify, ...
│   └── utils.ts
├── providers/
│   └── QueryProvider.tsx          # TanStack Query client provider
├── static/                        # markdown content for /about and /policies
├── store/
│   └── useVoteStore.ts            # Zustand store
└── types/
    └── index.ts
```

---

## Election status as a state machine

The entire UI branches on one value: `voting_status`. Every page checks it and redirects if the precondition is not met.

```mermaid
stateDiagram-v2
    [*] --> register : election created
    register --> vote_started : admin clicks Start Vote
    vote_started --> vote_ended : admin clicks End Vote

    register : /register active\n/vote blocked
    vote_started : /vote active\nadmin sees participation count
    vote_ended : results published\n/results/verify-vote active
```

---

## How voting works in the current implementation

The browser collects the voter's N1 and N2 codes and their candidate choice, then sends them to the backend over HTTPS. The backend handles the full cryptographic protocol — blind signing, RSA encryption, anonymization, and counting — server-side.

```mermaid
sequenceDiagram
    participant Browser
    participant Backend

    Browser->>Backend: POST /voters/check_n1 {n1}
    Backend-->>Browser: {is_N1_exist: true}
    Browser->>Backend: POST /voters/submit_vote {vote, n2}
    Backend-->>Browser: {status: "accepted"}
```

The blind signature protocol described in the [Cryptographic Protocol](/backend/cryptographic-protocol) page is the intended final design. In the current implementation, the backend performs the blinding, signing, and encryption operations on behalf of the voter.